...
function formatUsage(input_tokens: number, output_tokens: number): string {
  if (input_tokens > 1000 || output_tokens > 1000) {
    const inputFormatted =
      input_tokens > 1000
        ? `${(input_tokens / 1000).toFixed(1)}k`
        : `${input_tokens}`
    const outputFormatted =
      output_tokens > 1000
        ? `${(output_tokens / 1000).toFixed(1)}k`
        : `${output_tokens}`
...
async function getProjectThemeConfig(): Promise<{
  ...
  try {
    ...
    if (config.StatusLine) {
      ...
      // 检查是否有对应风格的配置
      if (
        config.StatusLine[currentStyle] &&
        config.StatusLine[currentStyle].modules
      ) {
...
export async function parseStatusLineData(
  ...
  try {
    ...
    const effectiveTheme =
      useSimpleTheme || !canDisplayNerd ? SIMPLE_THEME : DEFAULT_THEME

    // 获取主目录的主题配置，如果没有则使用确定的默认配置
    const { theme: projectTheme, style: currentStyle } =
      await getProjectThemeConfig()
...
// 读取用户主目录的主题配置（指定风格）
async function getProjectThemeConfigForStyle(
  style: string
): Promise<StatusLineThemeConfig | null> {
  try {
    // 只使用主目录的固定配置文件
    const configPath = CONFIG_FILE
    ...
    if (
      config.StatusLine &&
      config.StatusLine[style] &&
      config.StatusLine[style].modules
    ) {
...
function color256ToRgb(
  ...
  if (index < 16) {
    ...
    return {
      r: basicColors[index][0],
      g: basicColors[index][1],
      b: basicColors[index][2],
    }
  } else if (index < 232) {
    // 216色：6×6×6的颜色立方体
    const i = index - 16
