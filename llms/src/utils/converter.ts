...
export function convertToolsFromOpenAI(
  ...
  return tools.map((tool) => {
    // Handle both ChatCompletionTool and ChatCompletionCustomTool types
    ...
    // Fallback for other tool types or malformed tools
    throw new Error(`Unsupported tool type: ${tool.type || "unknown"}`)
  })
}
...
export function convertToOpenAI(
  ...
  if (toolResponsesQueue.size > 0) {
    toolResponsesQueue.forEach((responses: any, id: string) => {
      responses.forEach((response: any) => {
        messages.push(response)
      })
    })
...
export function convertToAnthropic(
  ...
  // Process messages to separate system message and convert tool responses
  const processedMessages = request.messages.filter(
    (msg) => msg.role !== "system"
  )
  ...
  for (let i = 0; i < processedMessages.length; i++) {
    ...
    if (msg.role === "tool") {
      // Find the previous assistant message that made this tool call
      let assistantIdx = i - 1
      while (
        assistantIdx >= 0 &&
        processedMessages[assistantIdx].role !== "assistant"
      ) {
        assistantIdx--
      }

      if (assistantIdx >= 0) {
        // Add tool result as a user message following the assistant's tool use
        messages.push({
          role: "user" as const,
          content: [
            {
      ...
      // Process regular user/assistant messages
      const content: any[] = []
      ...
      if (msg.tool_calls && msg.tool_calls.length > 0) {
        msg.tool_calls.forEach((toolCall) => {
          try {
            ...
            // If we can't parse the arguments, add it as a text block
            content.push({
              type: "text" as const,
              text: `Tool call: ${toolCall.function.name} with unparsable arguments`,
            })
          }
        })
      }
