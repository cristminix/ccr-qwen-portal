...
export class LLMService {
  ...
  async getAvailableModels(): Promise<any> {
    const providers = await this.providerService.getAvailableModels()

    return {
      object: "list",
      data: providers.data.flatMap((provider: any) => [
        {
          id: provider.id,
          object: "model",
  ...
  private getAvailableModelNames(): string[] {
    return this.providerService.getModelRoutes().map((route) => route.fullModel)
  }
