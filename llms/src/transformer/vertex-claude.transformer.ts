...
async function getAccessToken(): Promise<string> {
  try {
    ...
    throw new Error(
      "Failed to get access token for Vertex AI. Please ensure you have set up authentication using one of these methods:\n" +
  ...
  async transformRequestIn(
    ...
    if (!projectId && process.env.GOOGLE_APPLICATION_CREDENTIALS) {
      try {
        ...
        console.error(
          "Error extracting project_id from GOOGLE_APPLICATION_CREDENTIALS:",
          error
        )
      }
    }
    ...
    return {
      ...
      config: {
        url: new URL(
          `/v1/projects/${projectId}/locations/${location}/publishers/anthropic/models/${
            request.model
